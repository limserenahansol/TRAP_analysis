function trap_phase_fourway_zscore_plot(densMean, Node, GroupDelivery, GroupPhase, idvec, titleStr, pngPath, readmeTxt, nmax)
    idvec = idvec(:);
    ...
    for r = 1:n
        ...
        zrow = (row - mu) / sg;
        ac = zrow(GroupDelivery == "Active" & idxR);
        pc = zrow(GroupDelivery == "Passive" & idxR);
        aw = zrow(GroupDelivery == "Active" & idxW);
        pw = zrow(GroupDelivery == "Passive" & idxW);
    ...
    xtickangle(58);
    ylabel('Z-score (within region, across mice)');
