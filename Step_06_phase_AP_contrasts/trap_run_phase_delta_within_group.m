...
% Step 8: Within Active mice (and separately Passive): Reinstatement vs Withdrawal.
    ...
    root = C.phase_delta_within_group_root;
    ...
    if isfield(C, 'phase_AP_z_within_phase') && C.phase_AP_z_within_phase
        densMean = trap_zscore_within_phase_columns(densMean, pha);
        fprintf('Step 8: within-phase z per region (Step 3 scale); CSV deltas are z-units.\n');
        fid8 = fopen(fullfile(root, 'README_zscale_Step8.txt'), 'w');
        if fid8 > 0
            fprintf(fid8, '%s', 'delta_Rein_minus_With and means are z-scores within each phase per region.\n');
            fclose(fid8);
        end
    end
    r = pha == "Reinstatement";
    ...
    for i = 1:nReg
        ar = densMean(i, del == "Active" & r);
        ar = ar(isfinite(ar));
        aw = densMean(i, del == "Active" & w);
        aw = aw(isfinite(aw));
        pr = densMean(i, del == "Passive" & r);
        pr = pr(isfinite(pr));
        pw = densMean(i, del == "Passive" & w);
        pw = pw(isfinite(pw));
        nActR(i) = numel(ar);
        nActW(i) = numel(aw);
        nPasR(i) = numel(pr);
    ...
    plot_delta_block(densMean, del, pha, Node, tab, fig, 'Active_mice', 25, "Active", dAct, pAct, nActR, nActW);
    plot_delta_block(densMean, del, pha, Node, tab, fig, 'Passive_mice', 25, "Passive", dPas, pPas, nPasR, nPasW);
...
function plot_delta_block(densMean, del, pha, Node, tab, fig, gname, ntop, deliveryStr, dv, pv, nRv, nWv)
    ok = isfinite(dv);
    if nnz(ok) < 1
    ...
    trap_phase_plot_Rein_vs_With_mice_sem(densMean, del, pha, deliveryStr, Node, Tinc, ...
        ...
        fullfile(fig, sprintf('01_%s_top_increase_mice_sem.png', gname)), crit);
    trap_phase_plot_Rein_vs_With_mice_sem(densMean, del, pha, deliveryStr, Node, Tdec, ...
        ...
        fullfile(fig, sprintf('02_%s_top_decrease_mice_sem.png', gname)), crit);
end
