...
%
%   Outputs: TRAP_OUTPUT/06_phase_ActivePassive_FDR/phase_delta_screening/
%     - region_phase_delta_flip.csv (full table)
%     - figures_described: barh top regions by |flip|, scatter dRein vs dWith

    C = trap_config();
    root = fullfile(C.phase_AP_root, 'phase_delta_screening');
    ...
    if isfield(C, 'phase_AP_z_within_phase') && C.phase_AP_z_within_phase
        densMean = trap_zscore_within_phase_columns(densMean, GroupPhase);
    end
