...
% Optional userC struct overrides trap_config fields (e.g. phase_AP_root, phase_AP_row_filter_fn).
    ...
    if useZ
        densMean = densZ;
        fprintf('Within-phase z-score per region (Step 3 convention) — tables & bar/volcano plots.\n');
        fidz = fopen(fullfile(root, 'README_zscale_Step6.txt'), 'w');
        if fidz > 0
            fprintf(fidz, '%s', ['phase_stats_*.csv: mean_Active, mean_Passive, mean_Active_minus_Passive ' ...
                'are in **z-score units** (within each phase, per region across mice).\n' ...
                'Raw cells/mm³: four-way raw PNG + cohort CSVs.\n']);
            fclose(fidz);
        end
    else
        fprintf('Using raw density (cells/mm³). Set phase_AP_z_within_phase=true to match Step 3 z-scale.\n');
    end
    nR = size(densMean, 1);
    ...
    trap_phase_volcano_AP(T_rein, pass_rein, sprintf('Rein: all regions | %s', critStr), ...
        fullfile(fig0, '00_volcano_Reinstatement.png'), critStr, C);
    trap_phase_volcano_AP(T_with, pass_with, sprintf('Withdrawal: all regions | %s', critStr), ...
        fullfile(fig0, '00_volcano_Withdrawal.png'), critStr, C);
    trap_phase_barh_actpas_means(densMean, GroupDelivery, GroupPhase, "Reinstatement", Node, T_rein(pass_rein, :), ...
        sprintf('ALL sig Rein | mice+SEM | %s', critStr), fullfile(fig0, '01_ALL_sig_Rein_mice_sem.png'), critStr);
    trap_phase_barh_actpas_means(densMean, GroupDelivery, GroupPhase, "Withdrawal", Node, T_with(pass_with, :), ...
        sprintf('ALL sig Withdrawal | mice+SEM | %s', critStr), fullfile(fig0, '02_ALL_sig_With_mice_sem.png'), critStr);

    ntd = C.phase_AP_topN_direction_only;
    trap_phase_barh_actpas_topn_directional(densMean, GroupDelivery, GroupPhase, "Reinstatement", Node, T_rein, true, ntd, ...
        sprintf('Rein mean A>P top %d (direction only)', ntd), fullfile(fig0, '03_top25_Rein_Act_gt_Pas_direction_only.png'), critStr);
    trap_phase_barh_actpas_topn_directional(densMean, GroupDelivery, GroupPhase, "Withdrawal", Node, T_with, false, ntd, ...
        sprintf('With mean P>A top %d (direction only)', ntd), fullfile(fig0, '04_top25_With_Pas_gt_Act_direction_only.png'), critStr);
    trap_phase_barh_actpas_topn_directional(densMean, GroupDelivery, GroupPhase, "Withdrawal", Node, T_with, true, ntd, ...
        sprintf('With mean A>P top %d (direction only)', ntd), fullfile(fig0, '05_top25_With_Act_gt_Pas_direction_only.png'), critStr);
    ...
    trap_phase_volcano_AP(T_rein, sig_rein_A, sprintf('A1 Rein A>P | %s', critStr), ...
        fullfile(figA, '01_volcano.png'), critStr, C);
    trap_phase_barh_actpas_means(densMean, GroupDelivery, GroupPhase, "Reinstatement", Node, T_rein(sig_rein_A, :), ...
        sprintf('A1 Rein A>P | %s', critStr), fullfile(figA, '02_A1_Rein_mice_sem.png'), critStr);
    trap_phase_volcano_AP(T_with, sig_with_P, sprintf('A2 With P>A | %s', critStr), ...
        fullfile(figA, '03_volcano_Withdrawal.png'), critStr, C);
    trap_phase_barh_actpas_means(densMean, GroupDelivery, GroupPhase, "Withdrawal", Node, T_with(sig_with_P, :), ...
        sprintf('A2 With P>A | %s', critStr), fullfile(figA, '04_A2_With_mice_sem.png'), critStr);
    trap_plot_shared_regions_Rein_With_mice_sem(densMean, Node, GroupDelivery, GroupPhase, T_rein, T_with, idAsh, ...
        'Scenario A shared (sig)', figA, critStr, 'A_shared_sig');
    if ~isempty(idAsh)
    ...
    ntd = C.phase_AP_topN_direction_only;
    trap_phase_barh_actpas_topn_directional(densMean, GroupDelivery, GroupPhase, "Reinstatement", Node, T_rein, true, ntd, ...
        'Scenario A Rein A>P (direction only)', fullfile(figA, '07_top25_Rein_direction_only.png'), critStr);
    trap_phase_barh_actpas_topn_directional(densMean, GroupDelivery, GroupPhase, "Withdrawal", Node, T_with, false, ntd, ...
        'Scenario A With P>A (direction only)', fullfile(figA, '08_top25_With_direction_only.png'), critStr);
    ...
    trap_plot_shared_regions_Rein_With_mice_sem(densMean, Node, GroupDelivery, GroupPhase, T_rein, T_with, idAsh_dir, ...
        sprintf('Scenario A top-%d shared direction-only', min(ntd, max(1, numel(idAsh_dir)))), figA, critStr, 'A_shared_dirTopN');

    fprintf('Scenario A: Rein A>P n=%d | With P>A n=%d | Shared n=%d\n', ...
        nnz(sig_rein_A), nnz(sig_with_P), numel(idAsh));
    ...
    trap_phase_volcano_AP(T_rein, sig_rein_A, sprintf('B1 Rein A>P | %s', critStr), ...
        fullfile(figB, '01_volcano_Rein.png'), critStr, C);
    trap_phase_barh_actpas_means(densMean, GroupDelivery, GroupPhase, "Reinstatement", Node, T_rein(sig_rein_A, :), ...
        sprintf('B1 Rein A>P | %s', critStr), fullfile(figB, '02_B1_Rein_mice_sem.png'), critStr);
    trap_phase_volcano_AP(T_with, sig_with_A, sprintf('B2 With A>P | %s', critStr), ...
        fullfile(figB, '03_volcano_With.png'), critStr, C);
    trap_phase_barh_actpas_means(densMean, GroupDelivery, GroupPhase, "Withdrawal", Node, T_with(sig_with_A, :), ...
        sprintf('B2 With A>P | %s', critStr), fullfile(figB, '04_B2_With_mice_sem.png'), critStr);
    trap_plot_shared_regions_Rein_With_mice_sem(densMean, Node, GroupDelivery, GroupPhase, T_rein, T_with, idBsh, ...
        'Scenario B shared (sig)', figB, critStr, 'B_shared_sig');
    if ~isempty(idBsh)
    ...
    ntd = C.phase_AP_topN_direction_only;
    trap_phase_barh_actpas_topn_directional(densMean, GroupDelivery, GroupPhase, "Reinstatement", Node, T_rein, true, ntd, ...
        'Scenario B Rein A>P (direction only)', fullfile(figB, '07_top25_Rein_direction_only.png'), critStr);
    trap_phase_barh_actpas_topn_directional(densMean, GroupDelivery, GroupPhase, "Withdrawal", Node, T_with, true, ntd, ...
        'Scenario B With A>P (direction only)', fullfile(figB, '08_top25_With_direction_only.png'), critStr);
    ...
    trap_plot_shared_regions_Rein_With_mice_sem(densMean, Node, GroupDelivery, GroupPhase, T_rein, T_with, idB_dir, ...
        sprintf('Scenario B top-%d shared direction-only', min(ntd, max(1, numel(idB_dir)))), figB, critStr, 'B_shared_dirTopN');

    fprintf('Scenario B: Rein A>P n=%d | With A>P n=%d | Shared n=%d\n', ...
        nnz(sig_rein_A), nnz(sig_with_A), numel(idBsh));
