...
% Always writes raw_cells_mm3/ and z_within_phase/ under directional_AP_root.
    ...
    if isfield(C, 'phase_AP_flat_outputs') && C.phase_AP_flat_outputs
        root7 = baseRoot7;
        ...
        if ~isfield(C, 'phase_AP_plot_scale_label') || isempty(strtrim(char(string(C.phase_AP_plot_scale_label))))
            Cp.phase_AP_plot_scale_label = ternary_ap_str7(useZ, 'within-phase z', 'raw cells/mm³');
        else
            Cp.phase_AP_plot_scale_label = C.phase_AP_plot_scale_label;
        end
        lab = char(strtrim(string(Cp.phase_AP_plot_scale_label)));
        ...
        trap_AP_directional_run_one_scale(C, Cp, root7, densWork, densRaw, densZ, Node, GroupDelivery, GroupPhase, critStr, lab);
        fprintf('Step 7 (flat) -> %s\n', root7);
        return;
    end
    ...
    for iSc = 1:numel(scaleDirs)
        ...
        trap_AP_directional_run_one_scale(C, Cp, root7, densWork, densRaw, densZ, Node, GroupDelivery, GroupPhase, critStr, char(string(scaleDirs{iSc})));
    end
...
function trap_AP_directional_run_one_scale(C, Cp, root7, densWork, densRaw, densZ, Node, GroupDelivery, GroupPhase, critStr, scaleLogLabel)
    ...
    pack1(C, base1, '01_Reinstatement_Active_higher_than_Passive', T_rein, sig_rein_A, ...
    ...
    pack1(C, base1, '02_Withdrawal_Passive_higher_than_Active', T_with, sig_with_P, ...
    ...
    if ~isempty(idsFour1)
        if ~isempty(idAsh)
            t1r = 'S1 shared four-way raw';
            t1z = 'S1 shared z (Step 3 scale)';
        else
            t1r = 'S1 four-way raw (top-N direction-only; no sig.-threshold shared)';
            t1z = 'S1 four-way z (top-N direction-only; no sig.-threshold shared)';
        end
        trap_phase_fourway_plot(densRaw, Node, GroupDelivery, GroupPhase, idsFour1, ...
            t1r, fullfile(fd, '01_fourway_raw.png'), critStr, C.phase_AP_fourway_max);
        trap_phase_fourway_zscore_plot(densZ, Node, GroupDelivery, GroupPhase, idsFour1, ...
            t1z, fullfile(fd, '02_fourway_z.png'), critStr, C.phase_AP_fourway_max);
        trap_phase_shared_scatter_rein_with(T_rein, T_with, idsFour1, 'S1 scatter', fullfile(fd, '03_scatter.png'), '', 22);
    else
        trap_export_placeholder_figure(fullfile(fd, '01_fourway_skipped.png'), 'S1 four-way', ...
            'No sig.-threshold shared regions and no direction-only top-N list.');
    end
    ...
    if ~isempty(idS1dir) && (~isempty(idAsh) && (numel(idS1dir) ~= numel(idAsh) || ~isempty(setxor(idAsh(:), idS1dir(:)))))
        trap_phase_fourway_plot(densRaw, Node, GroupDelivery, GroupPhase, idS1dir(:), ...
            'S1 shared dirTopN four-way raw', fullfile(fd, '04_fourway_raw_dirTopN.png'), critStr, C.phase_AP_fourway_max);
        trap_phase_fourway_zscore_plot(densZ, Node, GroupDelivery, GroupPhase, idS1dir(:), ...
            'S1 shared dirTopN four-way z', fullfile(fd, '05_fourway_z_dirTopN.png'), critStr, C.phase_AP_fourway_max);
    end

    pack1(C, base2, '01_Reinstatement_Active_higher_than_Passive', T_rein, sig_rein_A, ...
    ...
    pack1(C, base2, '02_Withdrawal_Active_higher_than_Passive', T_with, sig_with_A, ...
    ...
    if ~isempty(idsFour2)
        if ~isempty(idBsh)
            t2r = 'S2 shared four-way raw';
            t2z = 'S2 shared z (Step 3 scale)';
        else
            t2r = 'S2 four-way raw (top-N direction-only; no sig.-threshold shared)';
            t2z = 'S2 four-way z (top-N direction-only; no sig.-threshold shared)';
        end
        trap_phase_fourway_plot(densRaw, Node, GroupDelivery, GroupPhase, idsFour2, ...
            t2r, fullfile(fd2, '01_fourway_raw.png'), critStr, C.phase_AP_fourway_max);
        trap_phase_fourway_zscore_plot(densZ, Node, GroupDelivery, GroupPhase, idsFour2, ...
            t2z, fullfile(fd2, '02_fourway_z.png'), critStr, C.phase_AP_fourway_max);
        trap_phase_shared_scatter_rein_with(T_rein, T_with, idsFour2, 'S2 scatter', fullfile(fd2, '03_scatter.png'), '', 22);
    else
        trap_export_placeholder_figure(fullfile(fd2, '01_fourway_skipped.png'), 'S2 four-way', ...
            'No sig.-threshold shared regions and no direction-only top-N list.');
    end
    ...
    if ~isempty(idS2dir) && (~isempty(idBsh) && (numel(idS2dir) ~= numel(idBsh) || ~isempty(setxor(idBsh(:), idS2dir(:)))))
        trap_phase_fourway_plot(densRaw, Node, GroupDelivery, GroupPhase, idS2dir(:), ...
            'S2 shared dirTopN four-way raw', fullfile(fd2, '04_fourway_raw_dirTopN.png'), critStr, C.phase_AP_fourway_max);
        trap_phase_fourway_zscore_plot(densZ, Node, GroupDelivery, GroupPhase, idS2dir(:), ...
            'S2 shared dirTopN four-way z', fullfile(fd2, '05_fourway_z_dirTopN.png'), critStr, C.phase_AP_fourway_max);
    end
...
function pack1(C, base, subname, Tfull, mask, ttl, critStr, Cp, densMean, GroupDelivery, GroupPhase, phaseName, Node)
    ...
    trap_AP_emit_ttest2_volcano_duplicate(C, Cp, Tfull, mask, ttl, fullfile(fd, '01_volcano_all_regions_highlighted.png'), critStr);
