...
% Step 7: mice + SEM + mean bars; L+R per mouse; p = ranksum on mice (same as Step 6).
    ...
    if isfield(C, 'phase_AP_z_within_phase') && C.phase_AP_z_within_phase
        densMean = densZ;
        fprintf('Step 7: within-phase z per region (Step 3 scale).\n');
    end
    ...
    pack1(base1, '01_Reinstatement_Active_higher_than_Passive', T_rein, sig_rein_A, ...
        sprintf('S1-01 Rein A>P | %s', critStr), critStr, C, densMean, GroupDelivery, GroupPhase, "Reinstatement", Node);
    pack1_top25_dir(base1, '01_Reinstatement_Active_higher_than_Passive', densMean, GroupDelivery, GroupPhase, ...
        "Reinstatement", Node, T_rein, true, 'S1-01 Rein top25 direction-only', critStr, C);
    pack1(base1, '02_Withdrawal_Passive_higher_than_Active', T_with, sig_with_P, ...
        sprintf('S1-02 With P>A | %s', critStr), critStr, C, densMean, GroupDelivery, GroupPhase, "Withdrawal", Node);
    pack1_top25_dir(base1, '02_Withdrawal_Passive_higher_than_Active', densMean, GroupDelivery, GroupPhase, ...
        "Withdrawal", Node, T_with, false, 'S1-02 With top25 direction-only', critStr, C);
    ...
    trap_plot_shared_regions_Rein_With_mice_sem(densMean, Node, GroupDelivery, GroupPhase, T_rein, T_with, idAsh, ...
        'S1 shared sig', fd, critStr, 'S1_shared_sig');
    if ~isempty(idAsh)
    ...
    trap_plot_shared_regions_Rein_With_mice_sem(densMean, Node, GroupDelivery, GroupPhase, T_rein, T_with, idS1dir, ...
        'S1 shared dirTopN', fd, critStr, 'S1_shared_dirTopN');

    %% S2
    pack1(base2, '01_Reinstatement_Active_higher_than_Passive', T_rein, sig_rein_A, ...
        sprintf('S2-01 Rein A>P | %s', critStr), critStr, C, densMean, GroupDelivery, GroupPhase, "Reinstatement", Node);
    pack1_top25_dir(base2, '01_Reinstatement_Active_higher_than_Passive', densMean, GroupDelivery, GroupPhase, ...
        "Reinstatement", Node, T_rein, true, 'S2-01 Rein top25 direction-only', critStr, C);
    pack1(base2, '02_Withdrawal_Active_higher_than_Passive', T_with, sig_with_A, ...
        sprintf('S2-02 With A>P | %s', critStr), critStr, C, densMean, GroupDelivery, GroupPhase, "Withdrawal", Node);
    pack1_top25_dir(base2, '02_Withdrawal_Active_higher_than_Passive', densMean, GroupDelivery, GroupPhase, ...
        "Withdrawal", Node, T_with, true, 'S2-02 With top25 direction-only', critStr, C);
    ...
    trap_plot_shared_regions_Rein_With_mice_sem(densMean, Node, GroupDelivery, GroupPhase, T_rein, T_with, idBsh, ...
        'S2 shared sig', fd2, critStr, 'S2_shared_sig');
    if ~isempty(idBsh)
    ...
    trap_plot_shared_regions_Rein_With_mice_sem(densMean, Node, GroupDelivery, GroupPhase, T_rein, T_with, idS2dir, ...
        'S2 shared dirTopN', fd2, critStr, 'S2_shared_dirTopN');

    fprintf('Step 7 -> %s\n', root7);
end

function pack1_top25_dir(base, subname, densMean, GroupDelivery, GroupPhase, phaseName, Node, Tphase, activeHigher, ttl, critStr, C)
    ...
    trap_phase_barh_actpas_topn_directional(densMean, GroupDelivery, GroupPhase, phaseName, Node, Tphase, ...
        activeHigher, C.phase_AP_topN_direction_only, ttl, fullfile(fd, '03_top25_direction_mice_sem.png'), critStr);
end

function pack1(base, subname, Tfull, mask, ttl, critStr, C, densMean, GroupDelivery, GroupPhase, phaseName, Node)
    sub = fullfile(base, subname);
    [fd, td] = subdirs(sub);
    Tsub = Tfull(mask, :);
    writetable(Tsub, fullfile(td, 'significant_regions.csv'));
    write_readme(sub, sprintf('%s\nn=%d\n', ttl, height(Tsub)));
    trap_phase_volcano_AP(Tfull, mask, ttl, fullfile(fd, '01_volcano_all_regions_highlighted.png'), critStr, C);
    trap_phase_barh_actpas_means(densMean, GroupDelivery, GroupPhase, phaseName, Node, Tsub, ...
        [ttl ' — mice+SEM'], fullfile(fd, '02_mice_sem_Act_vs_Pas.png'), critStr);
end
